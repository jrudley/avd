...
.SYNOPSIS
	v0.1.38
	Updated 10/30/22 - Use managed identities and convert disk sku's
#>
...
try {
    ...
    if (!$SkipAuth) {
        ...
        try {
            ...
            $AzAuth = Connect-AzAccount -Identity -EnvironmentName AzureUSGovernment
        ...
        Write-Log "Successfully authenticated with Azure the managed identity: $($AzContext | Format-List -Force | Out-String)"
    }
    ...
    if ($Ops.nVMsToStart -or $Ops.nCoresToStart) {
        ...
        foreach ($VM in $VMs.Values) {
            ...
            if ($PSCmdlet.ShouldProcess($SessionHostName, 'Start session host as a background job')) {
                ...
                Set-AvdDisk -rgName $vm.instance.resourcegroupname -vm $vm.instance -diskSku 'Premium_LRS'
                $StartVMjobs += ($VM.Instance | Start-AzVM -AsJob)
